...
# Datos de entrada
#=============================================================================
data = np.loadtxt('dato_marmo.txt')
...
wav = np.float32(np.hamming(nw)*espc[np.int16(ns/2)-np.int16(nw/2):np.int(ns/2)+np.int16(nw/2)+1])
...
R = np.loadtxt('R_marmousi.txt') # cargo la refle si la conozco

fig, ax = plt.subplots(1,3,figsize=(12,8))# base x altura
ax[0].set_title('Dato')
